...
async function persist() {
  ...
  // 本地文件备份（可选，本地开发时用）
  try {
    fs.writeFileSync(DB_FILE, dataStr);
  } catch (e) {
    console.error('file persist error:', e.message);
  }
...
// Initialize: async load from Supabase/file, otherwise create fresh
// (实际初始化在文件底部执行，确保所有路由和中间件已定义)
let _dataLoaded = false;
loadPersisted().then(async (loaded) => {
  if (!loaded) {
    console.log('no data found, initializing fresh data');
    initTeachers(DB);
    initStudents(DB);
